...
class STSocialLogin extends BaseController{
   ...
   public $createAuthUrl   =  "";
    ...
    public function __socialLogin()
    {
        ...
        if (empty($channel)) $this->sendError(esc_html__('Channel is missing', 'traveler-social-login'));
        switch ($channel) {
            case "facebook":
    ...
    public function twitterLoginCallBack()
    {
        if (class_exists("Abraham\TwitterOAuth\TwitterOAuth")) {
            if (empty($_GET['oauth_verifier'])) {
                echo esc_html__("Author Verifier ID is missing", 'traveler-social-login');
                die;
            }
    ...
    public function twitterLogin()
    {
        if (class_exists("Abraham\TwitterOAuth\TwitterOAuth")) {
            ...
            wp_redirect($url);
            echo esc_html__("Redirecting you to twitter. Please wait", 'traveler-social-login');
            die;
        }
    }
    ...
    protected function handleFacebookLogin()
    {
        ...
        if (!$access_token) $this->sendError(esc_html__('Access Token is missing', 'traveler-social-login'));
        ...
        if (is_wp_error($response)) {
            $this->sendError(esc_html__('Can not connect to Facebook. Please try again later', 'traveler-social-login'));
        }
        $body = wp_remote_retrieve_body($response);
        $json = json_decode($body, true);
        if (empty($json)) $this->sendError(esc_html__('Can not read Facebook response. Please try again later', 'traveler-social-login'));
        if (!empty($json['error'])) $this->sendError(esc_html__("Facebook Error: ", 'traveler-social-login') . $json['error']['message']);
        /**
        ...
        if ($register_user) {
            ...
            if (is_wp_error($user_id)) {
                $this->sendError(esc_html__('Error: ', 'traveler-social-login') . $user_id->get_error_message());
            }
    ...
    protected function handleGoogleLogin()
    {
         ...
         * @todo Kiểm tra khách có cấp quyền lấy email chưa
         */
        if (empty($user_email)) $this->sendError(esc_html__("Google Error: You must allow us read your email address ", 'traveler-social-login'));
        /**
    ...
    protected function getGoogleUserData($access_token)
    {
        ...
        if (empty($json) or empty($json['id'])) return new \WP_Error('api_error', esc_html__('Can not get user info', 'traveler-social-login'));
        return $json;
    }
    /**
    ...
    protected function getGoogleAccessToken($authorizationCode)
    {
        ...
        $json = json_decode($body, true);
        if (!empty($json['error_description'])) return new \WP_Error('api_error', $json['error_description']);
        if (empty($json) or empty($json['access_token'])) return new \WP_Error('api_error', esc_html__('Can not get access token', 'traveler-social-login'));
    ...
    protected function createOauthAccount($channel, $email, $oauth_id, $name = '')
    {
        ...
        if ($email_check) return new \WP_Error('api_error', esc_html__('Email exists', 'traveler-social-login'));
    ...
    protected function verifyRequest($action_name = 'st_frontend_security')
    {
        if (!$this->verifyNonce('_s', $action_name)) {
            $res = esc_html__('Your session has ended. Please reload the website', 'traveler-social-login');
