    ...
    class TravelHelper {
        ...
        static function init() {
            ...
            self::$all_currency = [
                ...
                'THB'  => 'Thailand Baht',
                'XAF'  => 'The Central African CFA franc',
        ...
        public static function add_menu_below($nav_menu, $args) {
            ...
            if ($menu_style == '3' && !is_user_logged_in() && $args->theme_location == 'primary') {
                ...
                <a href="#" class="login" '.esc_attr($data_toggle).'="modal"
                   '.esc_attr($data_target).'="#st-login-form">' . esc_html__('Login', 'traveler') . '</a>
            </li>' . '<li class="topbar-item signup-item">
                    <a href="#" class="signup" '.esc_attr($data_toggle).'="modal"
                       '.esc_attr($data_target).'="#st-register-form">' . esc_html__('Sign Up', 'traveler') . '</a>
                </li>';
                $nav_menu .= '</ul>';
            }
        ...
        static function st_approved_item_action($new_status, $old_status, $post) {
            if ($new_status != $old_status && $old_status != 'publish' && $new_status == 'publish') {
                if($post->post_type != 'st_order'){
                    if(in_array($post->post_type, array('st_hotel','hotel_room','st_activity','st_tours','st_rental'))){
        ...
        static function get_current_currency($need = false) {
            ...
            $data = self::get_currency_cookie('currency');
            if (isset($data['currency'])) {
                $name = $data['currency']['name'] ;
                if ($session_currency = self::find_currency($name)) {
                    if ($need and !empty($session_currency[$need])){
        ...
        static function change_current_currency($currency_name = false) {
            ...
            if ( !$change_currencyds) {
                self::set_currency('change_currencyds','');
            }


            if ( !empty( $_GET[ 'currency' ] ) and $new_currency = self::find_currency( $_GET[ 'currency' ] ) ) {
                self::set_currency('change_currencyds','ok');
                if(!empty($new_currency)){
					if ( st_check_is_checkout_woocomerce( false ) && is_plugin_active( 'woocommerce-currency-switcher/index.php' )){
						global $WOOCS;
						$WOOCS->set_currency( $_GET[ 'currency' ] );
					}
                    self::set_currency('currency',['currency'=>$new_currency]);
                }
        ...
        static function convert_money($money = false, $rate = false, $round = true) {
            if (!$money)
                $money = 0;
            if (!$rate) {
                $current_rate = !empty(self::get_current_currency('rate')) ? self::get_current_currency('rate') : 1;
                $current = self::get_current_currency('name');

                $default = self::get_default_currency('name');
                if(is_array($current_rate)) {
                    $current_rate = 1;
                }
                if ($current != $default){
        ...
        static function format_money($money = false, $need_convert = true, $precision = 0) {
            ...
            if(is_array($precision)){
                $precision_decimal = !empty($precision['booking_currency_precision']) ? $precision['booking_currency_precision'] : 0;
            } else {
            ...
            $money = number_format((float) $money, (int) $precision_decimal, $decimal_separator, $thousand_separator);
        ...
        static function format_money_raw($money = '', $symbol = false, $precision = 2, $template = null) {
            ...
            if (!empty($precision) && !empty($money)) {
        ...
        static function convert_money_from_to($money = '', $currency_from = '', $currency_to = '') {
            ...
            $currency_from_rate = !empty($currency_from['rate']) ? (float) $currency_from['rate'] : 1;

            $currency_to_rate = !empty($currency_to['rate']) ? (float) $currency_to['rate'] : 1;
        ...
        static function build_url_array_tree($key, $name, $value, $add = true, $post_type = '') {
            ...
            if (is_front_page() && !empty($post_type)) {
        ...
        static function convertDateFormat($date) {
            ...
            if($format === '{dd}/{mm}/{yyyy}'
                ...
                //if day/month/year then format to dd-mm-yyyy
                if(!empty($date)){
                    $format = self::getDateFormat();
                    $pos = strpos($date, ' ');
                    if ($pos)
                ...
                //if month/day/year then format to m/d/Y
                $format = self::getDateFormat();
                if (!empty($date)) {
                    //$date = str_replace('/', '-', $date);
                    $pos = strpos($date, ' ');
        ...
        static function getDateFormatMoment() {
            ...
            $format = apply_filters('getDateFormatMoments',$format);
            return $format;
        }
        ...
        static function paging($query = false, $wrapper = true) {
            ...
            if ($links) :
               ...
               $links = str_replace('<span', '<a href="#"', $links);
        ...
        static function paging_single_partner($query = false, $user_id = 0) {
        ...
        static function rate_to_string($star, $max = 5, $style ='') {
            $html = '';
            if(!empty($star)){
                if ($star > $max)
                    $star = $max;
                ...
                for ($i = 1; $i <= $moc1; $i++) {
                    ...
                    if(check_using_elementor()){
                        if($style == 'new'){
                            $html .= '<li class="list-unstyled"><i class="stt-icon-star1"></i></li>';
                        } else {
                            $html .= '<li class="list-unstyled"><i class="fa fa-star"></i></li>';
                        }
                ...
                if ($du >= 0.2 and $du <= 0.9) {
                    if(check_using_elementor()){
                        $html .= '<li class="list-unstyled"><i class="fas fa-star-half-alt"></i></li>';
                    } else {
                        $html .= '<li class="list-unstyled"><i class="fa fa-star-half-o"></i></li>';
                    }
                    ...
                    if(check_using_elementor()){
                        if($style == 'new'){
                            ...
                            $html .= '<li class="list-unstyled"><i class="far fa-star"></i></li>';
                        }

                    } else {
                        $html .= '<li class="list-unstyled"><i class="fa fa-star-o"></i></li>';
                    }
                ...
                for ($i = 1; $i <= $new; $i++) {
                    if(check_using_elementor()){
                        if($style == 'new'){
                            $html .= '<li class="list-unstyled"><i class="stt-icon-star2"></i></li>';
                        } else {
                            $html .= '<li class="list-unstyled"><i class="far fa-star"></i></li>';
                        }
        ...
        static function getListFullNameLocation($post_type = '', $limit = false) {
            if (array_key_exists('full_name_' . $post_type, self::$listFullNameLocations) && !$limit) {
                return self::$listFullNameLocations['full_name_' . $post_type];
            }
        ...
        static function get_room_price($room_id = false, $start_date = 'now', $end_date = 'now') {
        ...
        static function st_admin_notice_user_partner_check_approved() {
            ...
            if (count($data_user_register) > 0 or count($data_user_update_certificates) > 0) {
                ...
                if (count($data_user_register) > 0) {
                    echo sprintf(__('Have %d new user partner need check for approved.', 'traveler'), count($data_user_register));
        ...
        static function st_approved_item($author, $post) {
            ...
            $subject = __('You have a item is approved', 'traveler');

            global $author_approved, $post_approved;
            $author_approved = $author;
            $post_approved = $post;

            $id_page_email_approved = st()->get_option('email_approved', '');
            $email_approved = !empty(get_post($id_page_email_approved)) ? wp_kses_post(get_post($id_page_email_approved)->post_content) : "";
        ...
        static function _get_currency_book_history($post_id) {
            ...
            if ($st_is_woocommerce_checkout) {
                ...
                $st_currency = get_post_meta($post_id, '_order_currency', true);

                if (!empty($st_currency)) {
                    $get_currency = self::find_currency($st_currency);
                    if (isset($get_currency['symbol'])) {
        ...
        static function transferDestinationOptionNewFronend($not_label_select = '') {
            $return = [];
            if($not_label_select != 'not_label_select'){
                $return[] = [
                    'label' => __('---- Select ----', 'traveler'),
                    'value' => ''
                ];
        ...
        static function set_currency( $currency_name, $currency ) {
            $data_compress = base64_encode(gzcompress(addslashes(serialize($currency)), 9));
            TravelHelper::setcookie( $currency_name, $data_compress, time() + ( 86400 * 30 ) );
        }

        static function setcookie($name='', $value='', $expire = 86400) {
        ...
        static function getNewIcon($name = '', $color = '', $width = '', $height = '', $stroke = false) {
            ...
            return '<i class="input-icon st-border-radius field-icon fa">' . $icon . '</i>';
        }
        static function getNewIconV2($name = '', $color = '', $width = '', $height = '', $stroke = false) {

            if (!self::$fontsv2) {
                include_once get_template_directory() . '/v3/fonts/font-svg.php';
                if (isset($fontsv2)) {
                    self::$fontsv2 = $fontsv2;
                }
            }
            if (empty(self::$fontsv2)) {
                return '';
            }
            if (!isset(self::$fontsv2[$name])) {
                return '';
            }
            $icon = self::$fontsv2[$name];
            if (!empty($color)) {
                if ($stroke) {
                    $icon = preg_replace('/stroke="(.{7})"/', 'stroke="' . $color . '"', $icon);
                } else {
            ...
            return '<i class="st-border-radius field-icon">' . $icon . '</i>';
        }
        static function comment_form( $args = array(), $post_id = null ) {
            ...
            $defaults = array(
                ...
                'comment_notes_before' => sprintf(
                    ...
                    sprintf(
                        ...
                        __( 'Your email address will not be published.' , 'traveler')
                    ),
                    ( $req ? $required_text : '' )
                ),
                'comment_notes_after'  => '',
            ...
            ?>
            <div id="respond" class="comment-respond st-border-radius <?php echo esc_attr( $args['class_container'] ); ?>" data-toggle-section="st-review-form"
                 style="display: none">
                <?php
        ...
        static function pagination_comment($args = []) {
            $defaults = [
                'range' => 4,
                'total' => false,
                'previous_string' => !empty($args['prev_icon']) ? '<i class="'.esc_attr($args['prev_icon']).'"></i>' :  __('Prev', 'traveler'),
                'next_string' => !empty($args['next_icon']) ? '<i class="'.esc_attr($args['next_icon']).'"></i>' : __('Next', 'traveler'),
            ...
            if ($previous && (1 == $page)) {
                $echo .= '<li class="st-previous disabled"><a class="page-item" href="javascript:void(0);" title="' . esc_html__('previous', 'traveler') . '">' . balanceTags($args['previous_string']) . '</a></li>';
            }
            if ($previous && (1 != $page)) {
                $echo .= '<li class="st-previous"><a class="page-item" href="' . esc_url($previous) . '" title="' . esc_html__('previous', 'traveler') . '">' . balanceTags($args['previous_string']) . '</a></li>';
            }
            ...
            if ($next && ($count == $page)) {
                $echo .= '<li class="st-next disabled"><a class="page-item" href="javascript:void(0);" title="' . esc_html__('next', 'traveler') . '">' . balanceTags($args['next_string']) . '</a></li>';
            }
            if ($next && ($count != $page)) {
                $echo .= '<li class="st-next"><a class="page-item" href="' . esc_url($next) . '" title="' . esc_html__('next', 'traveler') . '">' . balanceTags($args['next_string']) . '</a></li>';
            }
            if (isset($echo))
