...
if (!class_exists('STHotel')) {
    class STHotel extends TravelerObject
    {
        ...
        public function get_relate_rooms($hotel_id, $room_id) {
            $arg = [
                'post_type' => 'hotel_room',
                'posts_per_page' => apply_filters('st_number_room_relate', 8),
        ...
        public function st_format_hotel_price()
        {
            if (STInput::request('action') == 'st_format_hotel_price') {
                ...
                if ($this->check_price_ajax()['pass_validate']) {
                    $check_price_data = $this->check_price_ajax();
                    $response['status'] = 1;
                    $response['price_html'] = !empty($check_price_data['price_html']) ? wp_kses_post($check_price_data['price_html']) : '';
                    $response['data'] = $check_price_data;
                    echo json_encode($response);
                    wp_die();
                } else {
        ...
        public function check_price_ajax()
        {
            ...
            if ($room_num_search * $children < $child_number) {
            ...
            return array(
                ...
                'price_html' => $html,
                'number_night' => sprintf(_n('%d night', '%d nights', $numberday, 'traveler'), $numberday),
                'sale_price' => TravelHelper::format_money($sale_price),
                'total_price' => $price_new_html
            );
        }
        ...
        public function __getMapFilterAjax()
        {
            ...
            if ($layoutRequest == '5' && $formatRequest == 'halfmap') {
                $isNew = true;
            }
            if ($query->have_posts()) {
                while ($query->have_posts()) {
                    ...
                    if (!empty($map_lat) and !empty($map_lng)) {
                        ...
                        if($isNew){
                            $data_map[$stt]['content_html'] = preg_replace('/^\s+|\n|\r|\s+$/m', '', stt_elementorv2()->loadView('services/hotel/components/map-popup'));
        ...
        public function st_filter_hotel_ajax()
        {
            ...
            $layoutRequest = STInput::request('version_layout');
            $formatRequest = STInput::request('version_format');
            if (in_array($layoutRequest, ['5', '6']) && in_array($formatRequest, ['halfmap', 'popup'])) {
                $isNew = true;
            }
            if ($query->have_posts()) {
                while ($query->have_posts()) {
                        ...
                        if ($version == 'elementorv2') {
                            ...
                            if($layoutRequest == 6){
                                if($style == 'list'){
                                    ...
                                    $classes = 'col-lg-4 col-md-4 col-12 item-service';
                                }
                            }else{
                                if(!empty($st_item_row)){
                                    ...
                                    if($layoutRequest == 5 && $style == 'list'){
                                        $classes = 'col-lg-12 item-service';
                                    } else {
                                        $classes = 'col-lg-6 col-md-6 col-12 item-service';
                                    }
                                }

                            }
                            echo '<div class="'. esc_attr($classes) .'">';
                            if($layoutRequest == 6 && $style == 'list'){
                                $changeLayout = STInput::request('change_layout_to');
                                if(!empty($changeLayout)) {
                                    echo stt_elementorv2()->loadView('services/hotel/loop/' . $changeLayout);
                                }else{
                                    echo stt_elementorv2()->loadView('services/hotel/loop/list-2');
                                }
                            }else {
                                echo stt_elementorv2()->loadView('services/hotel/loop/' . $style);
                            }
                            echo '</div>';
                        } else {
                            if ($fullwidth) {
            ...
            ob_clean();
            ob_end_flush();
            $count = balanceTags($this->get_result_string($version)) . '<div id="btn-clear-filter" class="btn-clear-filter" style="display: none;">' . esc_html__('Clear filter', 'traveler') . '</div>';
            //Map
            $map_icon = st()->get_option('st_hotel_icon_map_marker', '');
            if (empty($map_icon))
                $map_icon = get_template_directory_uri() . '/v2/images/markers/ico_mapker_hotel.png';
            ...
            if (isset($_REQUEST['location_lat']) && isset($_REQUEST['location_lng'])) {
               ...
               if (!empty($location_lat) && !empty($location_lng)) {
                   $map_lat_center = $location_lat;
                   $map_lng_center = $location_lng;
               }
            }
            $data_tmp = [
                'data_map' => $data_map,
        ...
        function featch_dataroom($hotel_id, $post_id, $post_name, $start, $end)
        {
            ...
            $sql = "SELECT
                ...
                AND _order.{$column} = {$post_id} AND _order.`status` IN ('complete', 'wc-completed')";
                    ...
                    foreach ($groupday as $key => $date) {
                        ...
                        foreach ($custom_price as $key => $val) {
                            if ($date[0] >= $val->check_in && $date[0] <= $val->check_out) {
                                $status = $val->status;
                                if ($price_by_per_person == 'on') {
                                    $_price_child_item = intval($child_number) * floatval($val->child_price);
                                    $_price_adule_item = intval($adult_number) * floatval($val->adult_price);
                                } else {
        ...
        function do_add_to_cart()
        {
            ...
            $require_guest_name = STInput::request('require_guest_name', '');
            if(empty($require_guest_name) || $require_guest_name !== 'no'){
                if (!st_validate_guest_name($room_id, $adult_number, $child_number, 0)) {
            ...
            $data = [
				...
				'total_bulk_discount' => $ori_price - $sale_price
            ];
        ...
        function get_cart_item_html($item_id = false)
        {
            ...
            if($page_style == '2') {
                return stt_elementorv2()->loadView('services/hotel/components/cart-item', ['item_id' => $item_id]);
            }else{
        ...
        function get_result_string($version = '')
        {
            global $wp_query, $st_search_query;
            if ($st_search_query) {
                $query = $st_search_query;
            ...
            if ($version == 'elementorv2') {
                ...
                if (!empty($location_id) and isset($get_post)) {
                    ...
                    $location = STInput::request('address', '');
                }
                if ($query->found_posts) {
                    $result_string = $location ? sprintf(_n('%s hotel found in %s', '%s hotels found in %s', $query->found_posts, 'traveler'), $query->found_posts, '<span>' . $location . '</span>') : sprintf(_n('%s hotel found', '%s hotels found', $query->found_posts, 'traveler'), $query->found_posts);
                } else {
                    $result_string = $location ? sprintf(esc_html__('No hotels found in %s', 'traveler'), '<span>' . $location . '</span>') : esc_html__('No hotels found', 'traveler');
                }
                return $result_string;
            } else {
                if (!empty($location_id) and isset($get_post)) {
                    $p1 = sprintf(esc_html__('%s: ', 'traveler'), get_the_title($location_id));
                } elseif (STInput::request('address')) {
                    $p1 = sprintf(esc_html__('%s: ', 'traveler'), STInput::request('address', ''));
                }
                if ($query->found_posts) {
                    if ($query->found_posts > 1) {
                        $p2 = sprintf(esc_html__('%s hotels found ', 'traveler'), $query->found_posts);
                    } else {
                        $p2 = sprintf(esc_html__('%s hotel found ', 'traveler'), $query->found_posts);
                    }
                } else {
                    $p2 = esc_html__('No hotel found', 'traveler');
                }
                // check Right to left
        ...
        function ajax_search_room()
        {
            ...
            if ($date_diff < 1) {
                $result = [
                    'status' => 0,
                    'html' => st()->load_template('layouts/modern/common/message', '', ['status' => 'danger', 'message' => __('Make sure your check-out date is at least 1 day after check-in.', 'traveler')]),
            ...
            if ($query->have_posts()) {
                while ($query->have_posts()) {
                    $query->the_post();
                    if (function_exists('check_using_elementor') && check_using_elementor()) {
                        $style = get_post_meta( $hotel_id, 'hotel_layout_style', true );
                        if($style == 4 || $style == 5){
                            $html = preg_replace('/^\s+|\n|\r|\s+$/m', '', stt_elementorv2()->loadView('services/hotel/loop/room_item')) ;
                        } else {
        ...
        function alter_search_room_query()
        {
            ...
            add_filter('posts_fields', [$this, '_room_change_post_fields']);
        ...
        function remove_search_room_query()
        {
            ...
            remove_filter('posts_fields', [$this, '_room_change_post_fields']);
        ...
        public
        static function _get_all_room_notavai_numberroom_numberadult($id_hotel, $number_room, $number_adult, $check_in_timestampe, $check_out_timestampe)
        {
            global $wpdb;
            $where = '';
        ...
        function _alter_search_query_ajax($where)
        {
            ...
            if (STInput::request('start') and STInput::request('end')) {
                ...
                $list = array();
                //Get all room unavaibility in check in and checkout
                $st_room = new STRoom();
                $get_unavailability_room_check =  $st_room->get_unavailability_room_new(  $hotel_id,$checkin_ymd, $checkout_ymd,$adult_num, $child_num, $number_room );
                if(!empty($get_unavailability_room_check)){
                    array_merge($list, $get_unavailability_room_check);
                }
        ...
        function _get_join_query($join)
        {
            ...
            if ($disable_avai_check == 'off') {
                $join .= " LEFT JOIN {$table} as tb ON {$wpdb->prefix}posts.ID = tb.parent_id AND status = 'available'";
                $join .= " LEFT JOIN {$table3} as tb3 ON (tb.post_id = tb3.post_id and tb3.`status` IN ('publish', 'private'))";
            }
            $join .= " LEFT JOIN {$table2} as tb2 ON {$wpdb->prefix}posts.ID = tb2.post_id";
            return $join;
        }
        ...
        function _get_where_query($where)
        {
            ...
            if ($check_in && $check_out) {
                ...
                $children_number = STInput::get('child_number', 0);
                if (intval($children_number) < 0) $children_number = 0;
                $number_room = intval(STInput::get('room_num_search', 1));
                ...
                if ($disable_avai_check == 'off') {
                    ...
                    $where .= " AND tb.check_in >= {$check_in_stamp} AND tb.check_out <= {$check_out_stamp} AND tb.number_booked < tb.number ";
                }
                                    ...
                                    SELECT ID FROM (
                                        SELECT $wpdb->posts.*,( 6371 * acos( cos( radians({$location_lat}) ) * cos( radians( mt1.meta_value ) ) *
                                                        ...
                                                        sin( radians( mt1.meta_value ) ) ) ) AS distance
                                                            ...
                                                            and mt2.meta_key = 'map_lng'
                                                            AND $wpdb->posts.post_status = 'publish'
                                                            AND $wpdb->posts.post_type = 'st_hotel'
                                                            AND $wpdb->posts.post_date < NOW()
        ...
        public function _change_posts_fields($fields)
        {
			...
			if (self::is_show_min_price()) {
				if ($disable_avai_check == 'off') {
					$fields .= ', min(CAST(tb2.min_price as DECIMAL)) as st_price, COUNT(tb.id) as total_available';
				} else {
					$fields .= ', min(CAST(tb2.min_price as DECIMAL)) as st_price';
				}
			} else {
				if ($disable_avai_check == 'off') {
					$fields .= ', min(CAST(tb2.price_avg as DECIMAL)) as st_price, COUNT(tb.id) as total_available';
				} else {
        ...
        public function _change_posts_groupby($groupby)
        {
			...
			if ( ( !$groupby or strpos($wpdb->posts . '.ID', $groupby) === false ) && ( isset($_REQUEST['price_range']) && !empty($_REQUEST['price_range']) ) ) {
				$groupby .= self::group_by_range();
				$groupby .= self::group_by_available( 'AND' );
			} else {
				if ( !$groupby or strpos($wpdb->posts . '.ID', $groupby) === false ) {
					$groupby .= self::group_by_available( 'HAVING' );
				}
		...
		function group_by_available( $condition ) {
			global $wpdb;
			...
			if ( $condition == 'HAVING' ) {
				$groupby = $wpdb->prepare(' HAVING total_available >=%d ', $diff+1);
			}
			if ( $condition == 'AND' ) {
				$groupby = $wpdb->prepare(' AND total_available >=%d ', $diff+1);
			}

			return $groupby;
		}

		function group_by_range() {
			global $wpdb;
        ...
        function _get_order_by_query($orderby)
        {
            ...
            if ($check = STInput::get('st_orderby')) {
                global $wpdb;
                $meta_key = st()->get_option('hotel_show_min_price', 'avg_price');
                if ($meta_key == 'avg_price') $meta_key = "price_avg";
